...
export default function Index({ user }) {
  return (user ? <ProfileHome profile={user.profile} /> : <p>No Profile</p>)
}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <p className="text-gray-600 dark:text-gray-100 text-sm font-medium pl-3">
                {profile.lastname} {profile.firstname} <span className="text-purple-500">Active since: {new Date(profile.created_at * 1000).toLocaleDateString()}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-5">
            ...
            <a href="javascript:void(0)">
              <p className="text-sm font-medium cursor-pointer text-indigo-500"
                onClick={() => {navigator.clipboard.writeText(profile.refcode); alert("copied to clip board")} }
              >
                copy code
              </p>
            </a>
