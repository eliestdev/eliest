...
function Index(props) {
    ...
    return (
        <>
            <div className="px-4 pb-4 pt-20 lg:justify-between lg:p-4 md:p-4 sm:pt-20 w-full">
                <div className="flex-1 min-w-0 p-4">
                    <h2 className="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate mb-2">
                        Quick Actions  </h2>
                </div>
                <hr />
                <div className="grid gap-4 sm:grid sm:h-32 sm:grid-flow-row sm:gap-8 sm:grid-cols-3 mt-10">
                   

                {
                     data&&(   data.account_verified == true ? (<></>):(<div className="relative shadow p-4">
                        <Link to="/agent/profile">
                            
                        <dt>
                            <div className="absolute flex items-center font-bold justify-center h-12 w-12 rounded-md bg-red-800 text-white">
                               
                            </div>
                            <p className="ml-16 text-lg leading-6 font-medium text-red-600">Actions required</p>
                        </dt>
...
function ActionItem({ title, caption, icon, link }) {
    return (
        <>
            <div className="relative shadow p-4" onClick={()=>{}}>
                <Link to={link}>
                <dt>
