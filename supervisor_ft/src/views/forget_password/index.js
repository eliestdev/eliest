...
function ForgetPassword({ setToken }) {
  ...
  let authenticate = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    const res = await fetch(`${process.env.REACT_APP_SUPERVISOR_URL}v1/resetpassword`, {
    ...
    if(data.status === "SUCCESS") dispatch(setAuthError("Check your mail for instructions on how to set your password!"));
  };
